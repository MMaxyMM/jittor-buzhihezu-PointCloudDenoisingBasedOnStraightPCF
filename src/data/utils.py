...
def validate_noise_scale_sampling(
    noise_min: float,
    noise_max: float,
    sampling: str = "uniform",
    mid_min: Optional[float] = None,
    mid_max: Optional[float] = None,
    low_prob: Optional[float] = None,
    mid_prob: Optional[float] = None,
    high_prob: Optional[float] = None,
) -> None:
    """Validate uniform or three-segment Laplace-scale sampling settings."""
    values = np.asarray([noise_min, noise_max], dtype=np.float64)
    if not np.isfinite(values).all() or not 0.0 < noise_min < noise_max:
        raise ValueError(
            "noise scale bounds must be finite and satisfy "
            f"0 < min < max, got [{noise_min}, {noise_max}]"
        )
    if sampling == "uniform":
        return
    if sampling != "three_segment":
        raise ValueError(
            "noise_sampling must be 'uniform' or 'three_segment', "
            f"got {sampling!r}"
        )

    if mid_min is None or mid_max is None:
        raise ValueError(
            "three_segment sampling requires noise_mid_min and noise_mid_max"
        )
    if not noise_min < mid_min < mid_max < noise_max:
        raise ValueError(
            "three_segment intervals must satisfy "
            "noise_min < noise_mid_min < noise_mid_max < noise_max, got "
            f"{noise_min} < {mid_min} < {mid_max} < {noise_max}"
        )

    probabilities = np.asarray(
        [low_prob, mid_prob, high_prob], dtype=np.float64
    )
    if not np.isfinite(probabilities).all() or (probabilities < 0.0).any():
        raise ValueError(
            "noise_low_prob/noise_mid_prob/noise_high_prob must be finite "
            "and non-negative"
        )
    if not np.isclose(probabilities.sum(), 1.0, rtol=0.0, atol=1e-8):
        raise ValueError(
            "noise sampling probabilities must sum to 1.0, got "
            f"{probabilities.sum():.12g}"
        )


def sample_noise_scale(
    noise_min: float,
    noise_max: float,
    sampling: str = "uniform",
    mid_min: Optional[float] = None,
    ...
    """Sample one Laplace scale without changing the legacy uniform path."""
    if validate:
    ...
    if sampling == "uniform":
        # Keep this as the same single NumPy call used by the original code.
        return float(rng.uniform(noise_min, noise_max))
